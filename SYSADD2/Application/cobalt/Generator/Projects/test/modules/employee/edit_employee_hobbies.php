<?php
//****************************************************************************************
//Generated by Cobalt, a rapid application development framework. http://cobalt.jvroig.com
//Cobalt developed by JV Roig (jvroig@jvroig.com)
//****************************************************************************************
require 'path.php';
init_cobalt('Edit employee hobbies');

if(isset($_GET['emp_id']) && isset($_GET['auto_id']))
{
    $emp_id = urldecode($_GET['emp_id']);
    $auto_id = urldecode($_GET['auto_id']);
    require 'form_data_employee_hobbies.php';
    $orig_emp_id = $emp_id;
}

if(xsrf_guard())
{
    init_var($_POST['btn_cancel']);
    init_var($_POST['btn_submit']);
    require 'components/query_string_standard.php';
    require 'subclasses/employee_hobbies.php';
    $dbh_employee_hobbies = new employee_hobbies;

    $object_name = 'dbh_employee_hobbies';
    require 'components/create_form_data.php';
    $arr_form_data['orig_emp_id'] = $_POST['orig_emp_id'];
    extract($arr_form_data);

    if($_POST['btn_cancel'])
    {
        log_action('Pressed cancel button');
        redirect("listview_employee_hobbies.php?$query_string");
    }


    if($_POST['btn_submit'])
    {
        log_action('Pressed submit button');

        $message .= $dbh_employee_hobbies->sanitize($arr_form_data)->lst_error;
        extract($arr_form_data);

        if($dbh_employee_hobbies->check_uniqueness_for_editing($arr_form_data)->is_unique)
        {
            //Good, no duplicate in database
        }
        else
        {
            $message = "Record already exists with the same primary identifiers!";
        }

        if($message=="")
        {
            $dbh_employee_hobbies->edit($arr_form_data);


            redirect("listview_employee_hobbies.php?$query_string");
        }
    }
}
require 'subclasses/employee_hobbies_html.php';
$html = new employee_hobbies_html;
$html->draw_header('Edit Employee Hobbies', $message, $message_type);
$html->draw_listview_referrer_info($filter_field_used, $filter_used, $page_from, $filter_sort_asc, $filter_sort_desc);
$html->draw_hidden('auto_id');
$html->draw_hidden('orig_emp_id');
$html->draw_controls('edit');

$html->draw_footer();
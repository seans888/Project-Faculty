...
if(xsrf_guard())
{
    ...
    if($_POST['btn_cancel']) 
    {
        ...
        redirect("listview_office_docs.php");
    }
    ...
    if($_POST['btn_submit'])
    {
        ...
        if($message=="")
        {
            ...
            $csv_name = $token . $_SESSION['user'] . '_office_docs_' . $timestamp . '.csv';
            $filename = TMP_DIRECTORY . '/' .  $csv_name;

            $csv_contents = $dbh_office_docs->export_to_csv();
...
$html->draw_header('CSV Exporter: Office Docs', $message, $message_type);

...
//$Hidden_Orig_Primary_Keys_Script, related to above, adds the hidden orig_primary_keys to the form data
$Hidden_Orig_Primary_Keys_Script = '';
...
for($a=0;$a<$field_count;$a++)
{
    if($field[$a]['Attribute']=='primary key' || $field[$a]['Attribute']=='primary&foreign key')
    {
        ...
        {
            $Hidden_Orig_Primary_Keys .= "\$html->draw_hidden('orig_{$field[$a]['Field_Name']}');\r\n";
            $Orig_Primary_Keys .= "    \$orig_{$field[$a]['Field_Name']} = \${$field[$a]['Field_Name']};\r\n";

            if($Hidden_Orig_Primary_Keys_Script != '')
            {
                $Hidden_Orig_Primary_Keys_Script .= "\r\n";
            }
            $Hidden_Orig_Primary_Keys_Script .= "    \$arr_form_data['orig_{$field[$a]['Field_Name']}'] = \$_POST['orig_{$field[$a]['Field_Name']}'];";
        }
        else
        {
...
EOD;
        ...
        {
            $Date_Controls_To_POST .= "\r\n        \$arr_form_data['$field_name'] = strtotime(\$$field_name);";
        }
...
$Upload_Controls_Script
    ...
    if(\$_POST['btn_submit'])
    {
        ...
        if({$dbh_name}->check_uniqueness_for_editing(\$arr_form_data)->is_unique)
        {

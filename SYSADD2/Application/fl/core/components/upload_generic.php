...
$orig_filename = basename(str_replace("\0",'',($_FILES[$file_upload_control_name]['name'])));

if(empty($_FILES[$file_upload_control_name]['name']))
{
    $extension = pathinfo($_POST[$existing_file_upload_control_name], PATHINFO_EXTENSION);
}
else
{
...
if($extension=='' && empty($_FILES[$file_upload_control_name]['name']))
{
...
if($allowed_extension && $_FILES[$file_upload_control_name]['size'] > $max_attachment_size)
{
...
elseif($allowed_extension && $_FILES[$file_upload_control_name]['error'] == 0)
{
    ...
    if(move_uploaded_file($_FILES[$file_upload_control_name]['tmp_name'], $upload_destination_file))
    {
        $$file_upload_control_name = $temp_filename;
    }
    else
    {
        $message .= "File (" . $orig_filename . ") was not uploaded due to server error. Please contact the system administrator.<br>";
    }
}
elseif($allowed_extension)
{
    if($orig_filename != '')
    {
        ...
        switch($_FILES[$file_upload_control_name]['error'])
        {
...
{
    $message .= 'File not uploaded: invalid file type. Please consult your system administrator for allowed file types.<br>';
}
...
if($empty_previous_file)
{
    ...
    if(isset($_POST[$existing_file_upload_control_name]))
    {
       $_POST[$existing_file_upload_control_name] = '';
    }
}

if(empty($$file_upload_control_name))
{
    //No file uploaded, so rely on whatever is already existing (if there is)
    $arr_form_data[$file_upload_control_name] = $_POST[$existing_file_upload_control_name];
    $$file_upload_control_name = $_POST[$existing_file_upload_control_name];
}
else
{
    //File uploaded successfully, form data should be updated
    $arr_form_data[$file_upload_control_name] = $$file_upload_control_name;
}
